...
for (i = 0; i < cp_pal.length; i++) {jobindex[i]=i;} 
}
function paintHelp(){if (cp_mp3[jobindex[indexGame-1]].trim()!="" || cp_ogg[jobindex[indexGame-1]].trim()!=""){
$("#help_div").html("<img tabindex='0' id='playSound' class='imaLeft' src='Cordialidad_Comunic_resources/media/' alt='Play' align='left'>"+ "<p>" +ConstruPalWords(cp_que[jobindex[indexGame-1]])+ "</p>");
...
if (correct) {score=score+scoreInc;timeAct = timeAct + timeBon;successes++;indexGame++;if (indexGame > cp_pal.length) {$("#ardoraActCanvas").attr("aria-label",messageOk);showMessage("Ok");} else {for (i=0;i<answers.length;i++){$("#b_" + String(answers[i])).remove();}posAns=0;answers=[];if (au.length>0){document.getElementById(au).pause();}au="";paintHelp();}
