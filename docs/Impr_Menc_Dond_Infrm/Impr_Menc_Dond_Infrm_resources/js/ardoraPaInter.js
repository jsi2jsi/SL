...
if (all){stopSound();$("#ardoraActCanvas").attr("aria-label",messageOk);showMessage("Ok");}else{var newArea=true;for (i=0;i<areaSolved.length;i++){if (areaSolved[i].localeCompare("N")==0){if (area[i].localeCompare(ind)==0){newArea=false;}}}if (newArea){indexGame++;paintHelp();}}
...
contexto.font = "bold 10px Verdana, Geneva, sans-serif";
...
for (var i=0; i<areaGame.length; i++) {ini=i;game.push(areaGame[ini]);areaGame[ini]="X";}
}
