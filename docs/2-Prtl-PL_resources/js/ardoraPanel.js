...
var orY=parseInt($("#txt"+itemSel).css("top"))+5+5;var orX=parseInt($("#txt"+itemSel).css("left"));var poS=atob(doneB[itemSel-1]);
...
if (successes==answers.length) {clearInterval(timeWrite);paintPoints();$("#ardoraActCanvas").attr("aria-label",messageOk); showMessage("Ok");} else {attempts++;if (tiAttempts) {if (attempts > attemptsMax) {$("#ardoraActCanvas").attr("aria-label",messageAttempts);showMessage("Attempts");} else {$("#ardoraActCanvas").attr("aria-label", messageError);showMessage("Error");}} else {$("#ardoraActCanvas").attr("aria-label", messageError);showMessage("Error");}}
}
