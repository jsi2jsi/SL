...
} else {$("#ardoraQuest").html(vHtml+"<img alt='Play' id='playSound' class='imaLeft' src='Metd_Eval-PL_resources/media/' alt='Sound' align='left'>"+ "<p>" + questionW[indexG] + "</p>");
...
for (i = 0; i < iW.length; i++) {var ind=i;wordsG[ind]=iW[i];cG[ind]=iWc[i];imageW[ind] = imaW[i];questionW[ind] = queW[i];audioW[ind] = auW[i];}}
function isCorrect(){
...
if (correct){indexG++;if (indexG == wordsG.length) {$("#ardoraActCanvas").attr("aria-label",messageOk); showMessage("Ok");} else {
