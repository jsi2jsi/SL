...
function randomSort(){
if (items==7){boardGame[2][4]="$&"}for (i = 0; i < pos.length; i++) {if (memoWords(pos[i]).localeCompare("-1") != 0) {var rRow = Math.floor(Math.random() * rows);var rCol = Math.floor(Math.random() * cols-1);if (rCol<0){rCol=0;}
while (boardGame[rRow][rCol].localeCompare("") != 0) {rCol = rCol + 1;if (rRow<2){if (rCol==cols) {rCol = 0;rRow = rRow + 1;if (rRow == rows) {rRow = 0;}}}
else{if (rCol == cols-1) {rCol = 0;rRow = rRow + 1;if (rRow == rows) {rRow = 0;}}}}boardGame[rRow][rCol] = itp[i];
rRow = Math.floor(Math.random() * rows);rCol = Math.floor(Math.random() * cols-1);if (rCol<0){rCol=0;}while (boardGame[rRow][rCol].localeCompare("") != 0) {
rCol=rCol+1;if (rRow<2){if (rCol == cols) {rCol = 0;rRow = rRow + 1;if (rRow == rows) {rRow = 0;}}}else{
if (rCol == cols-1) {rCol=0;rRow = rRow + 1;if (rRow == rows) {rRow = 0;}}}}boardGame[rRow][rCol] = pos[i];}}paintBoard();
}
function paintBoard(){
...
ansDo++;if (ansDo==items){score = score + scoreInc;$("#ardoraActCanvas").attr("aria-label",messageOk); showMessage("Ok");}}
